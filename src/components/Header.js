...
const pages = [
  ...
  {
    label: 'Blog post',
    href: '/articles/how-i-use-tailwind-to-design-from-scratch',
  },
  { label: 'Contact', href: '/contact' },
]
            ...
            <Menu as="div" className="relative">
              {({ open }) => (
                <>
                  <Menu.Button
                    className={clsx(
                      'group flex items-center font-medium duration-200 ease-in-out',
                      open
                        ? 'text-slate-900'
                        : 'text-slate-700  hover:text-slate-900'
                    )}
                  ...
                  <Menu.Items className="absolute right-0 z-20 mt-3 w-52 space-y-1 rounded-lg bg-white p-2.5 drop-shadow filter">
                    {pages.map((subLink, i) => (
                      <Menu.Item key={`${subLink.label}-dropdown-desktop`}>
                        <Link
                          href={subLink.href}
                          className={clsx(
                            'block py-3.5 px-5 font-medium',
