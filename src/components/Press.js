...
export function Press() {
  return (
    <section className="overflow-hidden bg-white py-16 sm:py-24 lg:pt-32">
      <Container>
        <div className="text-center">
          <h2 className="font-display text-4xl font-semibold text-slate-900 sm:text-5xl">
            Press and Interviews
          </h2>
          <p className="mx-auto mt-6 max-w-md text-lg leading-8 text-slate-700">
            Recognitions are like tattoos, you only show them off to people you
            want to impress.
          </p>
        </div>
        <div className="mx-auto mt-14 max-w-lg columns-1 gap-6 space-y-6 sm:mt-16 sm:max-w-2xl sm:columns-2 lg:mx-0 lg:max-w-none lg:columns-3 xl:gap-8 xl:space-y-8">
          {items.map((item, index) => (
            <div
              key={`press-item-${index}`}
              className="break-inside-avoid-column rounded-xl bg-slate-50/75 p-7 shadow-sm shadow-sky-100/50 ring-1 ring-slate-900/5 lg:p-10"
            >
              <div className="flex items-center gap-2 text-md text-slate-600">
                <item.icon className="h-4 w-4 text-slate-500" />
                {item.category}
              </div>
              <h3 className="mt-6 font-display text-lg font-medium text-slate-900 lg:text-xl">
                {item.title}
              </h3>
              <div className="mt-9 space-y-1">
                <Link
                  href={item.link.href}
                  className="group flex items-center gap-2 text-md font-medium text-sky-600 duration-200 ease-in-out hover:text-sky-700"
                >
                  {item.link.label}
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
